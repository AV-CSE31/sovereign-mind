...
class ZkWitness:
    ...
    def sign_action(self, action_data: dict[str, Any]) -> str:
        ...
        canonical_string = (
            f"{action_data.get('run_id')}:{action_data.get('node')}:{action_data.get('thought')}"
        )

        signature = hmac.new(
            self._private_key, canonical_string.encode(), hashlib.sha256
        ).hexdigest()
    ...
    def _verify_compliance(self, data: dict[str, Any]) -> bool:
        ...
        return not ("--force" in thought or "-rf" in thought)
