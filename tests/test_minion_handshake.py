...
from app.core.minion_orchestrator import MinionOrchestrator, ContextSyncError

@pytest.mark.asyncio
async def test_handshake_success():
    """Test that execution proceeds when handshake passes."""
    with patch("app.core.minion_orchestrator.AuditLogger.get_current_hash", new_callable=AsyncMock) as mock_hash:
        # Mock valid hash
