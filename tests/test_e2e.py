...
class TestRAG:
    ...
    def test_rag_retrieval_query(self, api_client):
        ...
        api_client.post(
            f"{BASE_URL}/v1/system/ingest",
            data={"content": "The FlashRank reranker uses MiniLM-L-12-v2 model for semantic reranking of search results."},
            headers={"Content-Type": "application/x-www-form-urlencoded"}
        )
        
        # Query about the content
        payload = {
            "messages": [{"role": "user", "content": "What model does FlashRank use for reranking?"}],
            "config": {"mode": "local", "depth": "fast"}
        }
        response = api_client.post(
            f"{BASE_URL}/v1/chat/completions",
...
class TestChatCompletions:
    ...
    def test_system_message(self, api_client):
        ...
        payload = {
            "messages": [
                {"role": "system", "content": "You are a helpful pirate. Always say 'Arrr!' at the start."},
...
class TestAgentEndpoints:
    ...
    def test_agent_with_session_id(self, api_client, vault_session):
        ...
        response = api_client.post(
            f"{BASE_URL}/v1/agent/run",
            data={
                "query": "What documents are in my vault?",
                "session_id": vault_session
            },
            headers={"Content-Type": "application/x-www-form-urlencoded"},
            timeout=TIMEOUT
